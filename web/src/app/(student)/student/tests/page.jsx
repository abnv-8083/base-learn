...
import { Search, Clock, AlertCircle, Upload, CheckCircle, FileText, Eye } from 'lucide-react';
...
export default function StudentTests() {
  ...
  const handleFileUpload = async (e, testId) => {
    ...
    if (file.type !== 'application/pdf') {
      toast.error('Only PDF files are allowed for tests.');
      return;
    }
    ...
    try {
      ...
      await axios.post(`/api/student/assessments/${testId}/submit`, formData, {
  ...
  const filteredSubmitted = filterStatus === 'All' 
    ? submittedTests 
    : submittedTests.filter(t => {
        if (filterStatus === 'Submitted') return t.studentStatus === 'Submitted' || t.studentStatus === 'submitted';
        if (filterStatus === 'Graded') return t.studentStatus === 'Graded' || t.studentStatus === 'graded';
        return true;
      });
      ...
      <div className="page-header" style={{ marginBottom: 'var(--space-6)' }}>
        <div className="page-header-inner" style={{ alignItems: 'flex-end' }}>
          <div>
            ...
            <p className="page-subtitle">Participate and evaluate your knowledge through scheduled assessments.</p>
          </div>
        </div>
      </div>

      <div style={{ display: 'flex', gap: '12px', marginBottom: '32px', borderBottom: '1px solid var(--color-border)', paddingBottom: '0' }}>
         <button 
           onClick={() => setActiveTab('view')}
           style={{ padding: '12px 24px', border: 'none', background: 'none', borderBottom: activeTab === 'view' ? '3px solid var(--color-primary)' : '3px solid transparent', color: activeTab === 'view' ? 'var(--color-primary)' : 'var(--color-text-secondary)', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s' }}
         >
           Pending Tests
         </button>
         <button 
           onClick={() => setActiveTab('submitted')}
           style={{ padding: '12px 24px', border: 'none', background: 'none', borderBottom: activeTab === 'submitted' ? '3px solid var(--color-primary)' : '3px solid transparent', color: activeTab === 'submitted' ? 'var(--color-primary)' : 'var(--color-text-secondary)', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s' }}
         >
           My Submissions
         </button>
      </div>
      ...
      {activeTab === 'view' ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
          {pendingTests.length === 0 ? (
             <div style={{ gridColumn: '1 / -1', padding: '100px 40px', textAlign: 'center', background: 'var(--color-bg)', borderRadius: '28px', border: '1px dashed var(--color-border)' }}>
                <CheckCircle size={48} color="#10b981" style={{ margin: '0 auto 16px', opacity: 0.5 }} />
                <h3 style={{ fontSize: '24px', fontWeight: '800', marginBottom: '12px' }}>All Caught Up!</h3>
                <p style={{ color: 'var(--color-text-secondary)', fontSize: '16px' }}>There are no pending tests for you at the moment.</p>
             </div>
            ...
            <div key={task._id} className="card hover-lift" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 0, border: '1.5px solid #f1f5f9', borderRadius: '24px', background: '#fff' }}>
              <div style={{ width: '100%', height: '100px', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #f59e0b, #ec4899)' }}>
              ...
              <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '20px', lineHeight: '1.6', fontWeight: '500' }}>{task.description || 'Participate and evaluate your knowledge through scheduled assessments.'}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
            {filteredSubmitted.length === 0 ? (
               <div style={{ gridColumn: '1 / -1', padding: '60px 40px', textAlign: 'center', background: 'white', borderRadius: '24px', border: '1px dashed var(--color-border)' }}>
                  ...
                  <p style={{ color: 'var(--color-text-secondary)', margin: 0 }}>There are no tests with the status: <strong style={{color:'var(--color-primary)'}}>{filterStatus}</strong></p>
               </div>
              ...
              <div key={task._id} className="card hover-lift" style={{ padding: 'var(--space-5)', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span className="badge" style={{ background: 'var(--color-bg)', color: 'var(--color-text-secondary)' }}>Test</span>
