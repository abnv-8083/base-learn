...
import { PlaySquare, Calendar, ClipboardList, Award, Sparkles, ArrowRight, Play, BookOpen, Radio, TrendingUp, FileText } from 'lucide-react';
...
function StatCard({ href, icon: Icon, value, label, color, grad }) {
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '22px 24px', border: '1px solid #e8edf5', boxShadow: '0 2px 12px rgba(15,23,42,0.05)', display: 'flex', gap: '16px', alignItems: 'center', position: 'relative', overflow: 'hidden', transition: 'transform 0.2s, box-shadow 0.2s', cursor: 'pointer' }}
        ...
        <div>
          <div style={{ fontSize: '30px', fontWeight: '900', color: '#0f172a', lineHeight: 1, letterSpacing: '-0.04em' }}>{value}</div>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#64748b', marginTop: '4px' }}>{label}</div>
        </div>
        <ArrowRight size={16} color={color} style={{ marginLeft: 'auto', opacity: 0.5 }} />
      </div>
    </Link>
  );
}
...
function AssessItem({ title, sub, href, linkLabel, color }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid #f1f5f9', transition: 'background 0.15s' }}
      ...
      <Link href={href} style={{ padding: '6px 14px', borderRadius: '10px', background: `${color}15`, color, fontSize: '12px', fontWeight: '800', textDecoration: 'none', flexShrink: 0, transition: 'background 0.2s' }}>
        {linkLabel}
      </Link>
    </div>
  );
...
export default function StudentDashboard() {
  ...
  const completionData = [{ name: 'Completed', value: stats.completion }, { name: 'Remaining', value: 100 - stats.completion }];
  const COLORS = ['#6366f1', '#e8edf5'];
  ...
  return (
    <div style={{ paddingBottom: '60px' }}>
      ...
      {/* ── Hero Banner ── */}
      <div className="dash-fade" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 55%, #0f172a 100%)', borderRadius: '28px', padding: '36px 40px', marginBottom: '32px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: '900', color: 'white', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
              {greeting}, <span style={{ color: '#a5b4fc' }}>{firstName}!</span>
            </h1>
            <p style={{ margin: '8px 0 0', fontSize: '15px', color: 'rgba(148,163,184,0.9)' }}>
              You're in <strong style={{ color: 'white' }}>{stats.batch?.name || 'your batch'}</strong>. Ready to learn today?
            </p>
          </div>
          <Link href="/student/recorded-classes" style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white', padding: '13px 24px', borderRadius: '14px', fontWeight: '800', fontSize: '14px', textDecoration: 'none', boxShadow: '0 6px 20px rgba(99,102,241,0.45)', transition: 'all 0.2s' }}
      ...
      {/* ── Stats Grid ── */}
      <div className="dash-stats-grid dash-fade" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '32px', animationDelay: '0.05s' }}>
      ...
      <div className="dash-fade" style={{ marginBottom: '32px', animationDelay: '0.1s' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#0f172a', letterSpacing: '-0.02em', margin: 0 }}>📽️ Recently Uploaded Classes</h2>
          <Link href="/student/recorded-classes" style={{ fontSize: '13px', fontWeight: '700', color: '#6366f1', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>View all <ArrowRight size={13} /></Link>
        </div>
        {dashboardData.recentVideos.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            {dashboardData.recentVideos.map(v => (
              <Link key={v._id} href={`/student/recorded-classes?videoId=${v._id}`} style={{ textDecoration: 'none', borderRadius: '20px', overflow: 'hidden', background: 'white', border: '1px solid #e8edf5', boxShadow: '0 2px 12px rgba(15,23,42,0.06)', display: 'flex', flexDirection: 'column', transition: 'all 0.2s' }}
                ...
                <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', background: 'linear-gradient(135deg,#1e1b4b,#312e81)', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'absolute', top: '10px', right: '10px', background: '#6366f1', color: 'white', padding: '3px 10px', borderRadius: '8px', fontSize: '10px', fontWeight: '800', letterSpacing: '0.04em' }}>NEW</div>
          ...
          <div style={{ padding: '48px', textAlign: 'center', background: 'white', borderRadius: '20px', border: '1px dashed #e2e8f0' }}>
            <BookOpen size={36} color="#cbd5e1" style={{ margin: '0 auto 12px' }} />
            <p style={{ color: '#94a3b8', fontWeight: '600', fontSize: '14px', margin: 0 }}>No new videos uploaded recently. Check your recorded classes for previous sessions!</p>
          </div>
        )}
      </div>

      {/* ── Assessments ── */}
      <div className="dash-2col dash-fade" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '32px', animationDelay: '0.15s' }}>
        {/* Assignments */}
        <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #e8edf5', overflow: 'hidden', boxShadow: '0 2px 12px rgba(15,23,42,0.05)' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #e8edf5', overflow: 'hidden', boxShadow: '0 2px 12px rgba(15,23,42,0.05)' }}>
          ...
          {dashboardData.assessments?.tests?.length > 0 ? dashboardData.assessments.tests.map(t => (
            <AssessItem key={t._id} title={t.title} sub={`${t.subject?.name || ''} • Scheduled: ${t.deadline ? new Date(t.deadline).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'N/A'}`} href="/student/tests" linkLabel="Start →" color="#10b981" />
          )) : (
      ...
      {/* ── Bottom Row: Live + Progress ── */}
      <div className="dash-2col dash-fade" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', animationDelay: '0.2s' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', border: '1px solid #e8edf5', overflow: 'hidden', boxShadow: '0 2px 12px rgba(15,23,42,0.05)' }}>
          <div style={{ padding: '14px 20px', background: 'linear-gradient(to right, #fef2f2, #fee2e2)', borderBottom: '1px solid #fca5a5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#ef4444', animation: 'livePulse 1.5s ease infinite' }} />
          ...
          <div style={{ padding: '12px' }}>
            {dashboardData.upcomingLive.length > 0 ? dashboardData.upcomingLive.map(lc => (
              <div key={lc._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', borderRadius: '14px', background: lc.status === 'ongoing' ? '#fee2e2' : '#f8fafc', marginBottom: '8px', border: lc.status === 'ongoing' ? '1px solid #fca5a5' : '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: lc.status === 'ongoing' ? '#ef4444' : '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {lc.status === 'ongoing' ? <Radio size={16} color="white" /> : <Calendar size={14} color="white" />}
                  </div>
                  <div>
                    <div style={{ fontWeight: '700', fontSize: '13px', color: '#0f172a' }}>{lc.topic}</div>
                ...
                <Link href={`/student/live-classes/${lc._id}`} style={{ padding: '6px 12px', borderRadius: '9px', background: lc.status === 'ongoing' ? '#ef4444' : '#6366f1', color: 'white', fontSize: '12px', fontWeight: '800', textDecoration: 'none' }}>
                  Join
                </Link>
              </div>
            )) : (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '56px', height: '56px', position: 'relative' }}>
                <svg width="56" height="56" style={{ transform: 'rotate(-90deg)' }}>
                  ...
                  <circle cx="28" cy="28" r="22" fill="none" stroke="#6366f1" strokeWidth="6" strokeDasharray={`${(stats.completion / 100) * 138} 138`} strokeLinecap="round" />
                </svg>
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: '900', color: '#6366f1' }}>{stats.completion}%</div>
              </div>
            </div>
          </div>
            ...
            {subjectData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={subjectData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false} style={{ fontSize: '11px', fontWeight: '700' }} />
                  <YAxis axisLine={false} tickLine={false} style={{ fontSize: '10px' }} />
                  <Tooltip cursor={{ fill: '#f1f5f9' }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', fontSize: '13px' }} />
