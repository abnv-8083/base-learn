...
export default function InstructorStudentAnalytics() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/instructor/students/${id}/analysis`);
  ...
  const handleAddNote = async () => {
    ...
    setAddingNote(true);
    try {
      const res = await axios.post(`/api/instructor/students/${id}/notes`, { note });
